...
namespace Tusfacturas.Sdk.Net.Services
{
    ...
    public class BillingService : Service, IBillingService
    {
        ...
        public BillingBatchResponse BillignInstantBatch(BillingAsyncBatchRequest billReq)
        {
            ...
            if(!billReq.Receipts.Any() || billReq.Receipts == null) throw new ArgumentException("La lista de comprobantes no puede estar vacía.");
            ...
            var tipoReferencia = billReq.Receipts.First().Tipo;

            if(!billReq.Receipts.All(x => x.Tipo == tipoReferencia)) throw new InvalidOperationException("Todos los comprobantes deben ser del mismo tipo. Ej: FACTURA A.");
            ...
            if(!billReq.Receipts.Any(x => tiposRequeridos.Contains(x.Tipo))) throw new InvalidOperationException("Debe haber al menos un tipo de comprobante requerido: FACTURA A, FACTURA B o FACTURA C.");

            //Ordenamos por numero ascendente

            billReq.Receipts.OrderBy(x => x.Numero);
        ...
        public async Task<BillingBatchResponse> BillingAsyncBatch(BillingAsyncBatchRequest billReq)
        {
            ...
            if (billReq.Receipts.Any(x => DateTime.Parse(x.Fecha) < DateTime.Now.Date)) throw new InvalidOperationException("Las fechas de las facturas deben ser posterior a hoy.");

            //El campo numero debe ser (0)
            if(billReq.Receipts.All(x => x.Numero != "0")) throw new ArgumentException("El campo número debe de ser 0.");

            //Debes enviar un "external_reference" de manera obligatoria y debería ser único.
            var externalReferences = billReq.Receipts.Select(x => x.ExternalReference).ToList();
            ...
            if(billReq.Receipts.All(x => x.Tipo.Contains("E"))) throw new InvalidOperationException("No puede enviar comprobantes de Tipo E.");
